...
float* DHT11_GetDatos(void){
	...
	{
		reset_timer();
		while(HAL_GPIO_ReadPin(GPIOA, DHT11_Pin) == GPIO_PIN_RESET){	//el comienzo es un bajo
		if (read_timer() >= 60){
			flag = read_timer();return NULL;}}
		data_ciclos_bajo[i] = read_timer();
		reset_timer();
		while(HAL_GPIO_ReadPin(GPIOA, DHT11_Pin) == GPIO_PIN_SET){	//mido el tiempo en alto
		if (read_timer() >= 100){flag = 4;return NULL;}}
