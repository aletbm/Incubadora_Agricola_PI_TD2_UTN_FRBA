...
static uint32_t alarma_temp_countdown = 0;
static uint32_t alarma_humedad_countdown = 0;
...
/*
 * Controla que la humedad y temperatura no esten fuera de rango por mas tiempo que tiempo_alarma
 * En caso de superar tiempo_alarma enciende el buzzer y envia un mensaje de telegram
 * Si pasado el tiempo tiempo_alarma_critica el sistema sigue sin poder regularse envia otro mensaje de telegram
 * tiempo_alarma y tiempo_alarma_critica estan definidos en config.h
 */
void control_alarma(uint32_t temperatura, uint32_t humedad){
	if(alarma_temp_countdown - HAL_GetTick() > tiempo_alarma){
		...
		else if(alarma_temp_countdown - HAL_GetTick() > tiempo_alarma_critica){
			if(estado_alarma_temp == 1){
	...
	else if(alarma_humedad_countdown - HAL_GetTick() > tiempo_alarma){
		...
		else if(alarma_humedad_countdown - HAL_GetTick() > tiempo_alarma_critica){
			if(estado_alarma_humedad == 1){
...
void control_parametros(uint32_t temperatura, uint32_t humedad)
{
    ...
    if (temperatura > temp_max) { //mucho calor
        encender_cooler();
        apagar_lampara();
    } else if (temperatura < temp_min) { // mucho frio
        apagar_cooler();
        encender_lampara();
    }
    ...
    // Control de humedad
    if (humedad > hum_max) { // mucha humedad
    	apagar_humidificador();
    	if(temperatura > temp_min) //Si no hace frio (control temp tiene mayor prioridad)
    		encender_cooler();
    } else if (humedad < hum_min) { // poca humedad
        encender_humidificador();
    	if(temperatura < temp_max) //Si no hace calor (control temp tiene mayor prioridad)
    		apagar_cooler();
    }
